...
- (BOOL)open {
    ...
    if(err != SQLITE_OK) {
        KCSLogError(@"error opening!: %d", err);
        return NO;
    }
        ...
        if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
            if (!triedFinalizingOpenStatements) {
                ...
                while ((pStmt = sqlite3_next_stmt(_db, nil)) !=0) {
                    KCSLogDebug(@"Closing leaked statement");
            ...
            KCSLogError(@"error closing!: %d", rc);
        }
    }
    while (retry);
...
- (int)busyRetryTimeout {
    KCSLogDebug(@"%s:%d", __FUNCTION__, __LINE__);
    KCSLogDebug(@"FMDB: busyRetryTimeout no longer works, please use retryTimeout");
    return -1;
}

- (void)setBusyRetryTimeout:(int)i {
    KCSLogDebug(@"%s:%d", __FUNCTION__, __LINE__);
    KCSLogDebug(@"FMDB: setBusyRetryTimeout does nothing, please use setRetryTimeout:");
}
...
- (BOOL)rekeyWithData:(NSData *)keyData {
    ...
    if (rc != SQLITE_OK) {
        KCSLogError(@"error on rekey: %d", rc);
        KCSLogError(@"%@", [self lastErrorMessage]);
    }
...
- (void)warnInUse {
    KCSLogWarning(@"The FMDatabase %@ is currently in use.", self);
...
- (BOOL)databaseExists {
    ...
    if (!_db) {
        ...
        KCSLogError(@"The FMDatabase %@ is not open.", self);
...
- (KCS_FMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    if (_traceExecution && sql) {
        KCSLogTrace(@"%@ executeQuery: %@", self, sql);
    }
    ...
    if (!pStmt) {
        ...
        if (SQLITE_OK != rc) {
            if (_logsErrors) {
                KCSLogError(@"DB Error: %d \"%@\"", [self lastErrorCode], [self lastErrorMessage]);
                KCSLogError(@"DB Query: %@", sql);
                KCSLogError(@"DB Path: %@", _databasePath);
            }
    ...
    if (dictionaryArgs) {
        ...
        for (NSString *dictionaryKey in [dictionaryArgs allKeys]) {
            ...
            if (namedIdx > 0) {
                ...
                KCSLogDebug(@"Could not find index for %@", dictionaryKey);
            }
        }
    }
    else {
        ...
        while (idx < queryCount) {
            ...
            if (_traceExecution) {
                if ([obj isKindOfClass:[NSData class]]) {
                    KCSLogTrace(@"data: %ld bytes", (unsigned long)[(NSData*)obj length]);
                }
                else {
                    KCSLogTrace(@"obj: %@", obj);
                }
    ...
    if (idx != queryCount) {
        KCSLogError(@"Error: the bind count is not correct for the # of variables (executeQuery)");
        sqlite3_finalize(pStmt);
        _isExecutingStatement = NO;
        return nil;
    }
...
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    if (_traceExecution && sql) {
        KCSLogTrace(@"%@ executeUpdate: %@", self, sql);
    }
    ...
    if (idx != queryCount) {
        KCSLogError(@"Error: the bind count (%d) is not correct for the # of variables in the query (%d) (%@) (executeUpdate)", idx, queryCount, sql);
        sqlite3_finalize(pStmt);
        _isExecutingStatement = NO;
        return NO;
    }
    ...
    if (SQLITE_DONE == rc) {
        ...
        if (_logsErrors) {
            KCSLogError(@"Error calling sqlite3_step (%d: %s) SQLITE_ERROR", rc, sqlite3_errmsg(_db));
            KCSLogError(@"DB Query: %@", sql);
        }
    }
        ...
        // uh oh.
        if (_logsErrors) {
            KCSLogError(@"Error calling sqlite3_step (%d: %s) SQLITE_MISUSE", rc, sqlite3_errmsg(_db));
            KCSLogError(@"DB Query: %@", sql);
        }
    }
    else {
        // wtf?
        if (_logsErrors) {
            KCSLogError(@"Unknown error calling sqlite3_step (%d: %s) eu", rc, sqlite3_errmsg(_db));
            KCSLogError(@"DB Query: %@", sql);
        }
    }
    ...
    if (closeErrorCode != SQLITE_OK) {
        if (_logsErrors) {
            KCSLogError(@"Unknown error finalizing or resetting statement (%d: %s)", closeErrorCode, sqlite3_errmsg(_db));
            KCSLogError(@"DB Query: %@", sql);
        }
    }
